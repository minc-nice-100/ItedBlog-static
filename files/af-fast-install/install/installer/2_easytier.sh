...
print_status() { echo -e "${YELLOW}[INFO] $1...${NC}"; }
...
[ "$(uname -m)" != "x86_64" ] && error_exit "Current architecture $(uname -m) is not supported. Only x86_64 supported."

available_space=$(df -m "${INSTALL_DIR%/*}" | awk 'NR==2 {print $4}')
[ "$available_space" -lt "$REQUIRED_SPACE" ] && error_exit "Insufficient disk space in ${INSTALL_DIR%/*} (required ${REQUIRED_SPACE}MB, available ${available_space}MB)."
...
cp -rv "${SCRIPT_DIR}/../easytier"/* "${INSTALL_DIR}/" || error_exit "File copy failed. Check source files."
...
chmod +x "${UPGRADER_SCRIPT}"

# Check config and upgrader
...
systemctl enable --now "${SERVICE_NAME}.service" || error_exit "Service start failed. Check logs: journalctl -u ${SERVICE_NAME}.service -n 50 --no-pager"

# Verify service status
sleep 2
systemctl is-active --quiet "${SERVICE_NAME}.service" || error_exit "Service is not running properly. Check logs: journalctl -u ${SERVICE_NAME}.service -n 100 --no-pager"
...
systemctl daemon-reload || error_exit "Failed to reload systemd for upgrade service"
...
print_success "Installation successful! EasyTier service is running."
